...
0                           & \beta^{(1)}       & - (\lambda^{(1)} + \lambda_{ECF}) & 0                         &0           &0                               &0               &0                          &\lambda_{ECF} \frac{V_{ECF}}{V_c^{(1)}}  \\
...
0 & 0 & 0 & 0 & 0                               & \beta^{(2)}       & -(\lambda^{(2)} + \lambda_{ECF})      & 0                         &\lambda_{ECF} \frac{V_{ECF}}{V_c^{(2)}}  \\
...
To get matrix $A$ explicitly, we solve for $a^{(1)}$ and $a^{(2)}$.


With \textit{MatLab}, we could compute all eigenvalues of A and store them in $E = \{e_j\}_{j \in \{1, ...,9\}}$, where $e_j \in \mathbb{C}$. Let $e_{max} = argmax_{\{e_j \in E\}} \{real(e_j)\}$, then if $real(e_{max}) > 0$, this system around the steady state is unstable. Otherwise, if $real(e_{max}) \le 0$, we could conclude that the system around such a steady state is stable. \\
